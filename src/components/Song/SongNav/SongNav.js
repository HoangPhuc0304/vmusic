...
import './SongNav.scss'
import { songNavHeaderAPI, songNavBodyAPI } from '../../../fakeAPI/api/SongNavAPI'
        ...
        <div className="song-nav">
            ...
            <div className="song-nav-header">
                {songNavHeaderAPI.map((item) => (
                    <Link to={item.link} key={item.id} style={{textDecoration: "none"}}>
                        <div 
                            className={`song-nav-item ${item.id === activeCurrent ? 'active' : ''}`}
                        >
                            <i className={item.icon}></i>
                            <span className="song-nav-name">{item.name}</span>
                            {item.description && <span className='song-nav-description'>{item.description}</span>}
                        </div>
                    </Link>
                ))}
            </div>
            <div className="song-nav-body">
                ...
                {songNavBodyAPI.map((item) => (
                    <div key={item.id} className="song-nav-item">
                        <i className={item.icon}></i>
                        <span className="song-nav-name">{item.name}</span>
                        {item.description && <span className='song-nav-description'>{item.description}</span>}
                    </div>
                ))}
                <div className="song-nav-notice">
                    <p>Listen to music without advertisements</p>
                    <button>Buy VIP</button>
                </div>
                <div className="song-nav-library">
                    <div className="song-nav-library-frame">
                        <h3>Library</h3>
                        <i className="bi bi-pencil"></i>
                    </div>
                    <div className="song-nav-library-item">
